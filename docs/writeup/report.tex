...
\begin{align*}
	V &\sim  Inv-W_q(\Psi,\nu)\\
	\beta|V &\sim N_{p \times q}\{\Lambda, V  \otimes \Omega^{-1}\}
\end{align*}
...
The above means that given data $X$, $Y$, and the prior parameters, we have computable formulas for the parameters of the posterior--which is the estimated distribution of $\beta$ and $V$. We can use our Normal Inverse Wishart sampler to take points from this posterior. \emph{lm.multivariable} covers all of this in a single subroutine, returning samples $B$ and $V$. The code for it is in the appendix, and next section shows a usage example.
...
We implemented a Matrix-Normal-Inverse-Wishart sampler and proved its use. It is definitely an improvement over what an R user would use before, but it is not as strikingly faster as one might hope.
