...
One approach is to find \emph{analytic} formula for the densities and moments. The marginal moments are easy to look up \cite{Wikipedia1}
...
More easily and reliably than that, we can sample $X,V \sim NIW$ from a sampler that we know should work: the naive algorithm. %TODO: <-- grammar%
  , and then use kernel density estimation.
  

\includegraphics[scale=.7]{density.correct.pdf}\\*
\emph{A correct histogram}

\includegraphics[scale=.7]{density.incorrect.pdf}\\*
\emph{An incorrect histogram}
...
Finally, we used a k-s test [CITE?] to numerically detect marginals which deviate from the correct value ($\alpha$ cut-off of $5%$. For example 
\begin{verbatim}
...
