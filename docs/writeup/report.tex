...
For one of these marginal distributions, we can look at the density function, we can look at the moments (which includes variances), and we can look at the Kolmogorov-Smirnov statistic which tests for deviation between distributions.

One approach is to find \emph{analytic} formula for the densities and moments. 
And the marginal distributions are \cite{???} some $t$ for the elements of $X$ and $InvGamma$ for the diagonals of $V$.
But the effort of proving the formulas correct is at least as hard as writing the code in the first place, and no where could we find anyone who had a clue what the distribution of the off-diagonals of $V$ were like.
More easily and reliably than analytic forms, we can sample $X,V \sim NIW$ from a sampler that we know should work and use that for comparison.
The naive algorithm we used is listed in the appendix, and is as pretty well straight out of the definition of the Inverse Wishart distribution.
From those samples, we use kernel density estimation to compute distributions, and use those for comparison.
...
Finally, we used a k-s test [CITE?] to numerically detect marginals which deviate from the correct value ($\alpha$ cut-off of $0.05$). For example 
\begin{verbatim}
...
